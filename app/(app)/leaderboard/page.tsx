  ...
  if (search) {
    ...
    return (
      <Shell search={search}>
        ...
        <UserList rows={results} me={me} showRank={false} />
      </Shell>
    );
  }
    ...
    <Shell search="">
      {total === 0 ? (
        ...
        <>
          ...
          <UserList rows={rows} me={me} showRank rankOffset={safeOffset} />
          {totalPages > 1 && (
    ...
    <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-zinc-500">
          Top players by rating. Updated as contests resolve.
        </p>
      </header>
...
function SearchBar({ initialValue }: { initialValue: string }) {
  return (
    <form method="get" action="/leaderboard" className="relative">
      <input
        type="text"
        name="q"
...
function Pagination({
  ...
  const prevHref = page > 1 ? `/leaderboard?page=${page - 1}` : null;
  const nextHref =
    page < totalPages ? `/leaderboard?page=${page + 1}` : null;
  return (

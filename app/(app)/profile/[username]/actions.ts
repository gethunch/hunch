...
export async function updateName(
  ...
  await updateUserName(me.id, firstName, lastName);
...
export async function updateEmail(emailIn: string): Promise<ActionResult> {
  ...
  if (email.toLowerCase() === (me.email ?? "").toLowerCase()) {
    if (me.pendingEmail) {
      await clearUserPendingEmail(me.id);
      for (const p of pathsToRevalidate(me.username)) revalidatePath(p);
    }
    return { ok: true };
  }
  ...
  const existingOwner = await findUserIdByEmail(email);
  if (existingOwner && existingOwner !== me.id) {
  ...
  try {
    await updateUserPendingEmail(me.id, email);
  } catch {
...
export async function updateAvatar(
  ...
  await updateUserAvatar(me.id, avatarUrl);
